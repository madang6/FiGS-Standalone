...
EOFPYTHON
    ...
    if [ -z "$MISSING_DEPS" ]; then
        ...
        # Separate torchtyping from other deps (it conflicts with tyro's typeguard requirement)
        OTHER_DEPS=$(echo "$MISSING_DEPS" | tr ' ' '\n' | grep -v torchtyping | tr '\n' ' ')
        NEEDS_TORCHTYPING=$(echo "$MISSING_DEPS" | grep -q torchtyping && echo "yes" || echo "no")

        echo "Running dry-run first..."
        if [ -n "$OTHER_DEPS" ]; then
            pip install --dry-run $OTHER_DEPS
        fi
        if [ "$NEEDS_TORCHTYPING" = "yes" ]; then
            echo "(torchtyping will be installed with --no-deps to avoid typeguard conflict)"
        fi
        echo ""
        read -p "Dry-run looks good? Install? (y/n) " -n 1 -r
        echo
        if [[ $REPLY =~ ^[Yy]$ ]]; then
            # Install non-torchtyping deps normally
            if [ -n "$OTHER_DEPS" ]; then
                pip install $OTHER_DEPS
            fi
            # Install torchtyping with --no-deps to preserve typeguard>=4 (required by tyro/nerfstudio)
            if [ "$NEEDS_TORCHTYPING" = "yes" ]; then
