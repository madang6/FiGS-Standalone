"""
VehicleRateMPC variant using MinTimeSnap trajectory solver.

This controller uses the more robust MinTimeSnap solver instead of the
minimum snap QP solver, making it better suited for camera-based trajectories
that may have challenging optimization landscapes.
"""
...
class VehicleRateMPCMinTimeSnap(BaseController):
    """
    Vehicle Rate MPC Controller using MinTimeSnap trajectory solver.

    This is a variant of VehicleRateMPC that uses the MinTimeSnap solver
    for trajectory planning instead of the minimum snap QP solver. This makes
    it more robust for trajectories that don't originate from RRT* or other
    classical trajectory planners (e.g., camera-based camera trajectories).
    """
    ...
    def __init__(self,
                 ...
                 use_RTI: bool = False,
                 kT: float = None,
                 use_l2_time: bool = False) -> None:

        """
        Constructor for the VehicleRateMPCMinTimeSnap class.

        Args:
            - course:       Name/Config Dict of the course (keyframes JSON).
            ...
            - use_RTI:      Use RTI flag for MPC solver.
        ...
        # Derived Parameters
        if type(course) is str:
            print("Padding trajectory for VehicleRateMPCMinTimeSnap.")
        ...
        # =====================================================================
        # Compute Desired Trajectory using MinTimeSnap
        # =====================================================================

        if type(course) is str:
            print("Using MinTimeSnap solver for trajectory optimization...")
            try:
                # Solve using MinTimeSnap (more robust than QP solver)
                mts = MinTimeSnap(
                    traj_config_pd,
                    hz=hz_ctl,
                    kT=kT,
                    use_l2_time=use_l2_time
                )

                # Get desired trajectory
                Tsd, FOd = mts.get_desired_trajectory()

                # Convert to tXU format using dynamics
                # MinTimeSnap returns time and flat outputs; convert to state-control form
                tXUd = th.TsFO_to_tXU(Tsd, FOd, drn_spec["m"], drn_spec["tn"])

                print(f"MinTimeSnap trajectory solved successfully")
                print(f"  Duration: {Tsd[-1]:.2f}s, Samples: {len(Tsd)}")

            except Exception as e:
                print(f"MinTimeSnap solver failed: {e}")
                import traceback
                traceback.print_exc()
                raise ValueError(
                    "MinTimeSnap trajectory optimization failed. "
                    f"Error: {e}\n"
                    "This may indicate the trajectory is infeasible or has numerical issues."
                )
        else:
        ...
        # Necessary Variables for Controller
        self.Nx = nx
        self.Nu = nu
        self.tXUd = tXUd
        self.Qk = Qk
        ...
        self.lbu = lbu
        self.ubu = ubu
        self.ns = int(hz_ctl / 5)  # Same as VehicleRateMPC
    ...
    def control(self,
        ...
        """
        Method to compute the control input for the VehicleRateMPCMinTimeSnap controller.
        Uses MinTimeSnap trajectory solver for robust trajectory planning.
        ...
        if self.use_RTI:
            ...
            try:
                ...
                print("Warning: VehicleRateMPCMinTimeSnap failed to solve OCP. Using previous input.")
                ucc = self.solver.get(0, "u")
        t2 = time.time()
    ...
    def get_ydes(self, tcr: float, xcr: np.ndarray) -> np.ndarray:
        ...
        if idxf < self.tXUd.shape[1]:
            ...
            print("Warning: VehicleRateMPCMinTimeSnap.get_ydes() padding trajectory. Increase your padding horizon.")
